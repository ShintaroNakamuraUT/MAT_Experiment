...
class MPERunner(Runner):
    ...
    def warmup(self):
        ...
        if self.use_centralized_V:
            ...
            share_obs = obs
        # obs のshape は16, 4, 28. share_obsのshapeは16, 4, 112. 16はここでは" all_args.n_rollout_threads "の数
    ...
    def eval(self, total_num_steps):
        ...
        if self.use_centralized_V:
            ...
            eval_share_obs = np.expand_dims(eval_share_obs, 1).repeat(
    ...
    def render(self):
        ...
        for episode in range(self.all_args.render_episodes):
            obs = envs.reset()
            if self.use_centralized_V:
                share_obs = obs.reshape(self.n_eval_rollout_threads, -1)
                share_obs = np.expand_dims(share_obs, 1).repeat(self.num_agents, axis=1)
            else:
                share_obs = obs
